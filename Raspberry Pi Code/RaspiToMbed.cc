...
using namespace std;
unsigned short int motorValue = 0; // motor value for fwd, rev, left, right, stop
unsigned short int LEDValue = 0; //Turns on the LED strip if machine is active

void UpdateValues() {
	ifstream MotorFile;
	MotorFile.open("MotorWrite.txt");
	MotorFile >> motorValue;
	MotorFile.close();
	
	ifstream LEDFile;
	LEDFile.open("LEDWrite.txt");
	LEDFile >> LEDValue;
	LEDFile.close();
...
int main(int argc, char ** argv) {
  ...
  while(1) {
	...
	string motVal = to_string(motorValue);
	const char* mot = motVal.c_str();
	write(fd, mot, 1);
	sleep(1);
	read(fd, buf, 1);
	...
	string LEDVal = to_string(LEDValue);
	const char* LED = LEDVal.c_str();
	write(fd, LED, 1);
	sleep(1);
	read(fd, buf, 1);
	printf("LED print value =%s\n\r", LED);
	printf("LED value = %s\n\r",buf);   //print in terminal window
  }
